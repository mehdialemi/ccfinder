...
public class Triangle extends SparkApp {
    ...
    public JavaPairRDD<Integer, int[]> getOrCreateFonl() {
        if (fonl == null)
            return createFonl().persist(StorageLevel.MEMORY_AND_DISK());
        return fonl;
    }
    ...
    public JavaPairRDD<Integer, int[]> createFonl() {
        ...
        int partitions = neighborList.getNumPartitions() * P_MULTIPLIER;
    ...
    public long triangleCount() {
        Long tc3 = createVertexTC().map(kv -> Long.valueOf(kv._2)).reduce((a, b) -> a + b);
        return tc3 / 3;
    }
